...
from STSSL_aug import sim_global
...
class TemporalHeteroModel(nn.Module):
    ...
    def forward(self, z1, z2):
        '''
        :param z1, z2 (tensor): shape nlvc, i.e., (batch_size, seq_len, num_nodes, feat_dim) [b,t,n,c]
        :return loss: loss of generative branch. [b,c,t,n]
        '''
        h = (z1 * self.W1 + z2 * self.W2).squeeze(1)
        s = self.read(h) # s: summary of h
...
class STEncoder(nn.Module):
    ...
    def forward(self, x0, graph):
        ...
        ## out block
        x = self.out_conv(x) # [b,c,t=1,n]
        x = self.dropout3(self.ln3(x.permute(0, 2, 3, 1))) # [b,c,t,n] -> [b,t=1,n,c]
        return x
...
class Pooler(nn.Module):
    ...
    def __init__(self, n_query, d_model, agg='avg'):
        ...
        self.softmax = nn.Softmax(dim=2) # softmax on the seq_length dim, [b,c,t,n]
    ...
    def forward(self, x):
        ...
        """
        x_in = self.align(x)[:, :, -self.n_query:, :] # [b,c,q,n]
        # calculate the attention matrix A using key x   
        A = self.att(x) # x: [b,c,t,n], A: [b,q,t,n]
        A = F.softmax(A, dim=2)

        # calculate region embeding using attention matrix A
        x = torch.einsum('nclv,nqlv->ncqv', x, A)
        x_agg = self.agg(x).squeeze(2) # ncqv->ncv [b,c,q,n] -> [b,c,n]
        x_agg = torch.einsum('ncv->nvc', x_agg) # [b,c,n] -> [b,n,c]
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # x: [b,t,n,c]
        x = torch.tanh(self.fc1(x.permute(0, 3, 2, 1))) # [b,t,n,c] -> [b,c,n,t]
        x = self.fc2(x)
        return x
