...
def sim_global(flow_data, sim_type='cos'):
    ...
    :param flow_data: tensor, original flow [b,t,n,c] or location embedding [b,n,c]
    :param type: str, type of similarity, attention or cosine. ['att', 'cos']
    :return sim: tensor, symmetric similarity, [n,n]
    """
...
def aug_topology(sim_mx, input_graph, percent=0.2):
    ...
    :param sim_mx: tensor, symmetric similarity, [n,n]
    :param input_graph: tensor, adjacency matrix without self-loop, [n,n]
    :return aug_graph: tensor, augmented adjacency matrix on cuda, [n,n]
    """    
    ...
    # edge_num是（207*207）的一半（21424），但 drop_prob是 edge_mask中TRUE的数量（21321），有可能不等，导致报错：a和p size 不一致导致
    if edge_num > drop_prob.shape[0]:
...
def aug_traffic(t_sim_mx, flow_data, percent=0.2):
    ...
    :param t_sim_mx: temporal similarity matrix after softmax, [t,b,n]
    :param flow_data: input flow data, [n,t,n,c]
    """
