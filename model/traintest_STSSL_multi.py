...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            y = y[:, args.horizon-1:args.horizon, :, :]
        ...
        if mode == 'test':
            ...
            if args.horizon == 3:
                logger.info('Horizon 15mins : mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae, mape, rmse))
            if args.horizon == 6:
                logger.info('Horizon 30mins : mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae, mape, rmse))
            if args.horizon == 12:
                logger.info('Horizon 60mins : mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mae, mape, rmse))
            ys_true, ys_pred = ys_true.permute(1, 0, 2, 3), ys_pred.permute(1, 0, 2, 3)
...
def traintest_model():  
    ...
    for epoch_num in range(args.epochs):
        ...
        for x, y in data_iter:
            ...
            reg_loss = masked_mae_loss(y_pred, y_true) # masked_mae_loss(y_pred, y_true)
...
parser.add_argument('-horizon', type=int, default=3, help='output sequence length 3/6/12')
